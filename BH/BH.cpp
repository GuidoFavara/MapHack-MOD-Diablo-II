...
void BH::Initialize()
{
	...
	if(!config->Parse()) {
		...
		if(!config->Parse()) {
			string msg = "Falta el archivo Ombu_Settings.\nAttempted to load " +
				path + "Ombu_Settings.cfg (falla).\nAttempted to load "+
				path + "Ombu_Settings.cfg (falla).\n\nDefaults loaded.";
			MessageBox(NULL, msg.c_str(), "Error al cargar Ombu_Settings", MB_OK);
		}
	}
	itemConfig = new Config("Ombu_Settings.cfg");
	itemConfig->Parse();
